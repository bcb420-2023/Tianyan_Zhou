...
### Volcano Plot
I use volcano plot to visualize amount of differentially expressed genes obtained by using the QLF model. My interest gene is IFI44, which is the second top-hit in the downstream expressed genes, it is shown as green located at the top left corner in the graph.
Most of them are blue because most are the down-regulated genes.

```{r}
# Find the row index of the IFI44 gene in the data frame
ifi44_row <- which(rownames(qlf_output_hits$table) == "IFI44")

# Create a scatter plot of log fold-change vs. negative log10 p-value
plot(qlf_output_hits$table$logFC, -log10(qlf_output_hits$table$PValue), 
     pch = 1, col = "black", 
     main = "Volcano plot of differential expression Genes of the GDM-exposured case and controls.",
     xlab = "log fold-change", ylab = "-log10 p-value",
     cex.main = 1)

# Add a red asterisk at the position of IFI44
points(qlf_output_hits$table$logFC[ifi44_row], -log10(qlf_output_hits$table$PValue[ifi44_row]), 
       pch = "*", col = "green", cex = 2)

# Highlight up- and down-regulated genes in red and blue, respectively
up_genes <- qlf_output_hits$table$PValue < 0.05 & qlf_output_hits$table$logFC > 0
points(qlf_output_hits$table$logFC[up_genes], -log10(qlf_output_hits$table$PValue[up_genes]), 
       pch = 1, col = "red")

down_genes <- qlf_output_hits$table$PValue < 0.05 & qlf_output_hits$table$logFC < 0
points(qlf_output_hits$table$logFC[down_genes], -log10(qlf_output_hits$table$PValue[down_genes]), 
       pch = 1, col = "blue")

# Add a legend
legend("topright", 
       legend = c("Up-regulated", "Down-regulated", "neither", "IFI44"),
       fil = c("red", "blue", "black", "green"), 
       cex = 1.2)
...
It is worth noticing that the results of query using the down-regulated genes and the results of query using both the up and down-regulated genes are almost identical. This is maybe due to the fact that there are only 20 up regulated but 381 down-regulated genes in the lists, which means most of them are down-regulated genes and this causes the most results have a larger tendency to be reflect on the down-regulated genes. 


### results using both the upregulated and downregulated genes:
The number of genesets returned and the top-hit term:
GO:BP: 4028, with negative regulation of viral process (16)
reactome: 657, with  Interferon alpha/beta signaling(17)   
WikiPathways: 377, with Type I interferon induction and signaling during SARS-CoV-2 infection(10)
